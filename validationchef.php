...
<body>
  ...
  <center>
    ...
    <?php
      ...
      $pass="";
      $id=$_SESSION['mat'];
      $id_service=$_SESSION['idservice'];

      try{
        $connexion = new PDO("mysql:host=$serveur;dbname=gestionconge",$login,$pass);
        $connexion->setattribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
        $requete1=$connexion->prepare("
          SELECT NOM ,PRENOM ,DATE_DEBUT,DATE_FIN,TYPE_CONGE,SOLDE,ID_CONGE,ETAT,MATRICULE FROM CONGE NATURAL join(AGENT) where MATRICULE != $id AND ID_SERVICE=$id_service ORDER BY ID_CONGE DESC");
        $requete1->execute();
          ...
          if ($requete1[$i][7]== 1 || $requete1[$i][7]== -1) {
            echo"<td></td>";
             ...
             <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
              ...
              <a class="dropdown-item" href="Refuser.php?ID='.$arraycng[$i].' & S='.$arraysolde[$i].' & MAT='.$MAT.' & E=0 ">Refuser</a>
