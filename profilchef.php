      ...
      session_start();
        ...
        $connexion->query('SET NAMES utf8');
				$requete1=$connexion->prepare("
					SELECT MATRICULE,NOM,PRENOM,POST,NOM_SERVICE ,ID_SERVICE FROM AGENT natural join service where MATRICULE=$id");
				$requete1->execute();
				...
				$PRENOM=$requete1[0][2];
				$POST=$requete1[0][3];
        $SERVICE=$requete1[0][4];
        $ID_SERVICE=$requete1[0][5];
        $_SESSION['idservice']=$requete1[0][5];//pour validation chef//
        
        $requete2=$connexion->prepare("
          SELECT NOM_DIVISION FROM division natural join service where ID_SERVICE=$ID_SERVICE");
        $requete2->execute();
